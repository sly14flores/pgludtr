...
function doesDateExist($dtrs,$data,$date) {
	...
	foreach ($dtrs as $i => $dtr) {
		...
		if ($dtr['date'] == $date) return array("date"=>true,"dtr"=>$data[$i]);
...
$user = $con->getData("SELECT (SELECT divisions.division_shortname FROM divisions WHERE division_id = personal_infos.employee_division) office FROM personal_infos WHERE employee_no = ".$_SESSION['employee_no']);
foreach ($user as $value) {
	if ($value['office'] != null) $office = $value['office'];
}
...
$employee = $con->getData("SELECT UPPER(CONCAT(employee_lastname, ', ', employee_firstname, ' ', employee_name_ext, IF(employee_name_ext='','',' '), employee_middlename)) employee FROM personal_infos WHERE employee_id = '$id'");

$sql = "SELECT pers_id, date, morning_in, morning_out, afternoon_in, afternoon_out, overtime_in, overtime_out FROM daily_time_records WHERE pers_id = '$id' AND date LIKE '$datef%'";
...
$employees[] = array("empid"=>$id,"employee"=>$employee[0]['employee'],"data"=>$_data,"dates"=>$_dates,"travel_orders"=>$travel_orders,"leaves"=>$leaves);
...
function Header()
{
	...
	$this->Image("../img/pglu-watermark-colored.png",30,55,160);
...
function table($header,$id,$employee,$data,$dtr,$travel_orders,$leaves)
{
...
foreach($employees as $key => $value) {
	...
	$pdf->AddPage();
	$pdf->table($header,$value['empid'],$value['employee'],$value['data'],$value['dates'],$value['travel_orders'],$value['leaves']);
